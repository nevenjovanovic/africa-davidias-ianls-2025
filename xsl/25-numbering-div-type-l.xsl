...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:template match="//tei:text/tei:body/tei:div/tei:l">
        <xsl:element name="{local-name()}" namespace="http://www.tei-c.org/ns/1.0">
            <xsl:attribute name="n" xmlns:tei="http://www.tei-c.org/ns/1.0"><xsl:value-of select="../@n"/><xsl:text>.</xsl:text><xsl:value-of select="count(preceding-sibling::tei:l) + 1"/></xsl:attribute> 
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
     -->
    <xsl:template match="//tei:body//tei:div/tei:div[tei:l]/tei:l">
             ...
             <xsl:element name="{local-name()}" namespace="http://www.tei-c.org/ns/1.0">
                 ...
                 <xsl:attribute name="n" xmlns:tei="http://www.tei-c.org/ns/1.0"><xsl:value-of select="../@n"/>.<xsl:text></xsl:text><xsl:number level="single" count="tei:l" format="1"/></xsl:attribute>
                 <!-- <xsl:attribute name="n" xmlns:tei="http://www.tei-c.org/ns/1.0"><xsl:number level="multiple" count="tei:div[tei:l]/tei:l" format="1.1"/></xsl:attribute> -->
                 <xsl:apply-templates/>
             </xsl:element>
    </xsl:template> 
